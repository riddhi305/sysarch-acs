/** 
 * Verify S-EL2 physical timer (CNTHPS) mapping (PPI).
 */
...
payload(void)
{
    ...
    if (intid == 0) intid = PPI_RECOMMENDED_CNTHPS;
    val_print(ACS_PRINT_TEST, " S-EL2 virtual timer INTID (expected 20) = %ld", intid);
...
g02_entry(uint32_t num_pe)
{
    ...
    num_pe = 1;  //This PPI test is run on single processor
